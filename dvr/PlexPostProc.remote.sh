...
# The if selection statement proceeds to the script if $1 is not empty.
   ...
   FILENAMENORM=$(printf '%q\n' "$FILENAME")
   TEMPFILENAME="$(mktemp -p /tmp/).mkv"  # Temporary File for transcoding
   ...
   echo "********************************************************"
   ssh -tt root@server "ffmpeg -threads 12 -i "$FILENAMENORM" -s hd720 -c:v libx264 -preset veryfast -vf yadif -c:a copy "$TEMPFILENAME""
   check_errs $? "Failed to convert using ffmepg"

   echo "********************************************************"
   echo "Cleanup / Copy $TEMPFILENAME to $FILENAMENORM"
   echo "********************************************************"

   rm -f "$FILENAMENORM" # Delete original
